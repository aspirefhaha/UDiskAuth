...
void CAuthDlg::sltConfirm()
{
    ...
    if(enuret == SDR_OK){
        ...
        if(SDR_OK == sd_GetCapcity(hDevice,&cap)){
			...
			if (sd_ReadData(hDevice, cap - (REVSECCOUNT * 8 / 10), sizeof(tmpdata), (UCHAR *)tmpdata) == SDR_OK) {
				if (tmpdata[0] == 0x5a && tmpdata[1] == 0xa5) {
					if (QMessageBox::warning(this, tr("Operation Warning"),
						tr("Already Authorized,Continue?"),
